...
class Sound : Identifiable, ObservableObject {
    ...
    init(id : UUID, waves : [SingleWave], timeInBed : Double, started : Date, stoped : Date, fileName : String, inDayCound : Int, soundDuration : Double){
        self.id = id
        self.waves = waves
    ...
    init(id : UUID, samples : [Float], timeInBed : Double, started : Date, stoped : Date, fileName : String, inDayCound : Int, soundDuration : Double){
        self.id = id
...
extension Sound {
    ...
    static var data = [Sound(id: UUID(), samples: floats, timeInBed: 30 * 60, started: Date().addingTimeInterval(-3600 * 2.7), stoped: Date(), fileName: "", inDayCound: 1, soundDuration: 30 * 60)]
}
